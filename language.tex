...
\item \(C\) is a sequence of actions, which contains \(\Startconst\),
  \(\Endconst\), \(\sassx\) and \(\sassxn\). 
\end{itemize}
...
\begin{eqnarray*}
 \rho \mbox{ (label)} &::=& \Malloc \mid \Free \mid \tau %% \mid \sassx \mid \sassxn
...
{H(R(x)) = \NULL, \scon\Sirx \in C \mbox{ and } \sassxn \notin C }
{\langle H \coma R \coma \IFNULL\Sirx \ \THEN   s_{1}\ \ELSE\  s_{2} \coma  n, C \rangle
  \rightarrow_{D}
...
{H(R(x)) \neq \NULL, \scon\Sirx \in C \mbox{ and } \sassx \notin C}
{\langle H \coma R \coma \IFNULL\Sirx\ \THEN  s_{1}\ \ELSE  s_{2} \coma  n, C\rangle
  \rightarrow_{D}
