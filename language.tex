...
\item \rn{Sem-ConstSeq}: \(\scon\Sirx\) and \(\Endconst\) together
  guarantees a pointer pointed to by \(*x\) cannot be changed in
  statement \(s\). The set \(C\) with added \(\scon\Sirx\) describes
  this status.
\item \rn{Sem-IfConstNullT}: under the environment, i.e., \(*x\) cannot be
  changed and \(*x\) is a null pointer, 
...
In order to deal with a path-sensitive program to guarantee \emph{total} memory-leak freedom, we redefined the several definitions as follows.
defined as follows:
